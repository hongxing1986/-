...
W = {'D1':0.55,'D2':0.4,'D3a':7,'D3b':-1,'D4':6,
     'D5a':30,'D5b':27,'D5c':23,'D5d':19,
     'D6a':-10,'D6b':-6,'D6c':-2,
...
print()
print("="*60)
print("回测统计（第51-86期，共{}期）".format(total-50))
print("="*60)
start = 50
top1_hit = 0
top2_hit = 0
for i in range(start, total):
    ...
    t1, s1, t2, s2 = predict(train)
    
    if t1 in actual_set: top1_hit += 1
    if t2 in actual_set: top2_hit += 1

n = total - start
print()
print("TOP1命中率: {}/{} = {:.1f}%".format(top1_hit, n, top1_hit/n*100))
print("TOP2命中率: {}/{} = {:.1f}%".format(top2_hit, n, top2_hit/n*100))
print("TOP1+TOP2覆盖率: {}/{} = {:.1f}%".format(top1_hit+top2_hit, n, (top1_hit+top2_hit)/n*100))

# 预测最新一期
print()
print("="*60)
print("第{}期预测结果".format(total+1))
print("="*60)
t1, s1, t2, s2 = predict(all_data)
print()
print("  第一名: {} (得分:{:.1f})".format(t1, s1))
print("  第二名: {} (得分:{:.1f})".format(t2, s2))
print()
print("  重点关注: {}、{}".format(t1, t2))
