...
def predict(data, w=BASE_W):
    ...
    # D14: 生肖关联性
    zodiac_pairs = [('鼠','龙'), ('龙','蛇'), ('蛇','马'), ('马','羊'), ('羊','猴'),
                    ('猴','鸡'), ('鸡','狗'), ('狗','猪'), ('猪','牛'), ('牛','虎'),
                    ('虎','兔'), ('兔','鼠')]
    last_z = data[-1]['zodiacs']
    for z1, z2 in zodiac_pairs:
        if z1 in last_z: scores[z2] += 3
        if z2 in last_z: scores[z1] += 3
    ...
    even = 7 - odd
    if odd >= 5:
        for z in ['马','虎','龙','蛇','狗']: scores[z] += 5
    elif even >= 5:
        for z in ['鼠','牛','兔','羊','猴','鸡','猪']: scores[z] += 5
    
    # D16: 大小比例
    big = sum(1 for n in last_nums if n >= 25)
    small = 7 - big
    if big >= 5:
        for z in ['马','蛇','龙','虎','牛']: scores[z] += 4
    elif small >= 5:
        for z in ['鼠','兔','羊','猴','鸡','狗','猪']: scores[z] += 4
    
    # D17: 和值范围
    s = sum(data[-1]['numbers'])
    if 100 <= s <= 150:
        for z in zodiacs: scores[z] += 2
