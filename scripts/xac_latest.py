...
# ==================== 预测函数 ====================
def predict(data, skip_d15=False, d5_weight=30):
    """
    预测函数
    skip_d15: True=去掉D15(适合TOP2), False=保留D15(适合TOP1)
    d5_weight: D5维度权重，默认30
    """
...
if __name__ == '__main__':
    ...
    for i in range(50, len(all_data)):
        ...
        # TOP1: 有D15
        pred1, _ = predict(train, skip_d15=False, d5_weight=30)
        # TOP2: 去D15
        pred2, _ = predict(train, skip_d15=True, d5_weight=30)
        
        if pred1[0] in actual_set: top1_hit += 1
        if pred2[1] in actual_set: top2_hit += 1
    
    print(f"\nTOP1命中率(有D15): {top1_hit}/{n_backtest} = {top1_hit/n_backtest*100:.1f}%")
    print(f"TOP2命中率(去D15): {top2_hit}/{n_backtest} = {top2_hit/n_backtest*100:.1f}%")
    ...
    # TOP1预测(有D15)
    pred1, scores1 = predict(all_data, skip_d15=False, d5_weight=30)
    print(f"\n【TOP1预测】(有D15策略)")
