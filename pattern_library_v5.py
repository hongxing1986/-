...
class PatternLibrary:
    def __init__(self, strategies, recent_weight=3.0, feature_weights=None):
        ...
        self.feature_weights = feature_weights or {
            'repeat': 1.0, 'concentration': 1.2, 'miss_volatility': 1.0,
            'hot_ratio': 1.1, 'hot5_level': 1.0, 'last_in_hot': 1.0,
            'pos_hot': 1.5, 'pos_count': 1.2,
        }
    ...
    def predict(self, train, pos=None):
        name, match_type, key = self.get_prediction(train, pos)
...
if __name__ == '__main__':
    ...
    print('='*80)
    print('规律库预测系统 v5.0 - 扩展到近3-10期')
    print('='*80)
    print()
    print(f'策略总数: {len(strategy_funcs)}个 (近3-10期 × 4种条件)')
    print()

    # 构建规律库
    library = PatternLibrary(strategy_funcs, recent_weight=3.0)
    library.build(data)
    print(f'规律库: {len(library.library)}个特征组合')
    print()
    ...
    print('回测结果（第61-88期）')
    print('='*80)
    print('{:<8} | {:^12} | {:^8} | {}'.format('期数', '策略', '预测', '结果'))
    print('-'*80)
    ...
    for i in range(60, 88):
        ...
        pred, strat, match, key = library.predict(train, pos=0)
        hit = pred in actual
        if hit: hits += 1
        total += 1
        print('{:<8} | {:^12} | {:^8} | {}{}'.format(
            '第{}期'.format(i+1), strat, pred, '✓' if hit else '✗', 
            '' if hit else ' '+','.join(sorted(actual)[:4])))

    print()
    print('='*80)
    print(f'全期命中率: {hits}/{total} = {hits/total*100:.0f}%')
    ...
    print(f'近10期命中率: {recent_hits}/10 = {recent_hits/10*100:.0f}%')
    ...
    for strat, cnt in sorted(strat_count.items(), key=lambda x: x[1], reverse=True):
        print(f'  {strat}: {cnt}次 ({cnt/28*100:.0f}%)')
